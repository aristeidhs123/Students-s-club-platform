...
export async function show_pendingClubs() {
    const [pendingfrm] = await pool.query('SELECT * FROM club WHERE status="pending"')
    return pendingfrm[0]
}
...
export async function newmembership(iduser, idclub) {
    const [newmembr] = await pool.query('INSERT INTO membership(userid, clubid, membership_role, membership_status) VALUES (?, ?, "user", "pending")',[iduser, idclub])
    return newmembr
}

export async function show_pendingMemberships() {
    const [pendingmemb] = await pool.query('SELECT * FROM membership WHERE membership_status="pending"')
    return pendingmemb[0]
}

export async function show_approvedMemberships() {
    const [approvedmemb] = await pool.query('SELECT * FROM membership WHERE membership_status="approved"')
    return approvedmemb[0]
}

export async function show_rejectMemberships() {
    const [rejectmemb] = await pool.query('SELECT * FROM membership WHERE membership_status="rejected"')
    return rejectmemb[0]
}
